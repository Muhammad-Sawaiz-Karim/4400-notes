# 4400 notes (some useful links)  

## CUDA  
- FireShip CUDA video: [Link](https://youtu.be/pPStdjuYzSI)
- CUDA Programming Guide: [Link](https://docs.nvidia.com/cuda/cuda-c-programming-guide/)
- Grid-Stride Loop: [Link](https://developer.nvidia.com/blog/cuda-pro-tip-write-flexible-kernels-grid-stride-loops/)

## Prolog
- Short tutorial to Prolog: [Link](https://users.aber.ac.uk/ais/Prolog/)
- Unification: [Link](https://lpn.swi-prolog.org/lpnpage.php?pagetype=html&pageid=lpn-htmlse5)
- SWI-Prolog docs: [Link](https://www.swi-prolog.org/pldoc/man?section=quickstart)

## Lambda Calculus
- A Tutorial Introduction to the Lambda Calculus: [Link](https://personal.utdallas.edu/~gupta/courses/apl/lambda.pdf)
- University of Wisconsin-Madison Lambda Caclulus Notes: [Link](https://pages.cs.wisc.edu/~horwitz/CS704-NOTES/1.LAMBDA-CALCULUS.html)